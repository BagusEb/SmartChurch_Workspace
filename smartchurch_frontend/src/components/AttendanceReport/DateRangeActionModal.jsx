...
export default function DateRangeActionModal({
  show,
  title,
  subtitle,
  Icon,
      ...
      >
        <div className="flex justify-between items-start mb-5">
          <div className="flex items-center gap-3">
            <div className={`flex justify-center items-center ${iconGradient} rounded-xl w-10 h-10`}>
              {createElement(Icon, { size: 18, className: 'text-white' })}
            </div>
            <div>
              <p className="font-bold text-slate-800">{title}</p>
              <p className="mt-0.5 text-slate-400 text-xs">{subtitle}</p>
            </div>
          </div>
          {!isLoading && (
        ...
        <div className={`flex gap-2 ${infoBgClass} mb-4 p-3 rounded-xl`}>
          {createElement(Icon, { size: 15, className: `mt-0.5 ${infoIconClass} shrink-0` })}
          <p className={`${infoTextClass} text-xs leading-relaxed`}>
            {infoText}
          </p>
        </div>

        {error && (
          <p className="mb-3 text-rose-600 text-sm">{error}</p>
        )}
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`flex flex-1 justify-center items-center gap-2 ${primaryButtonClass} disabled:opacity-50 py-2.5 rounded-xl font-semibold text-white text-sm transition-opacity`}
          >
            {isLoading ? (
              <>
              ...
              <>
                {createElement(PrimaryIcon, { size: 15 })}
                {primaryLabel}
              </>
            )}
          </button>
