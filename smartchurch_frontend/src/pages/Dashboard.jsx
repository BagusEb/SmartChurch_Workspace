...
export default function GuestValidation() {
  ...
  const handleConfirmVerify = async () => {
    ...
    try {
      ...
      const attendanceWasSkipped = Boolean(
        result?.attendance_skipped || result?.already_attended
      );

      showToast(
        attendanceWasSkipped
          ? result?.message ||
              `${
                result?.member?.full_name ||
                finalMember?.full_name ||
                "Member"
              } sudah hadir pada session ini.`
  ...
  const handleConfirmMember = async () => {
    ...
    try {
      ...
      showToast(
        attendanceWasSkipped
          ? result?.message ||
              `${totalEmbeddings} data wajah berhasil ditambahkan ke ${
                result?.member?.full_name || "jemaat"
              }. Member sudah hadir sehingga attendance tidak diubah.`
