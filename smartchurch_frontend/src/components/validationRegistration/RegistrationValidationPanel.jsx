...
export default function RegistrationValidationPanel({ onAfterChange }) {
  ...
  const removeProcessedFacesFromData = (data, processedIds = []) => {
    if (!data) return data;

    const idSet = new Set(processedIds.map((id) => String(id)));

    const oldFaces = data.registration_faces || data.embeddings || [];
    const nextFaces = oldFaces.filter((face) => !idSet.has(String(face.id)));

    const nextTotalPending = Math.max(
      Number(data.summary?.total_pending_embeddings || 0) - processedIds.length,
      0
    );

    return {
      ...data,
      registration_faces: nextFaces,
      embeddings: nextFaces,
      summary: {
        ...(data.summary || {}),
        total_pending_embeddings: nextTotalPending,
        page_pending_embeddings: nextFaces.length,
      },
      pagination: {
        ...(data.pagination || {}),
        total_items: nextTotalPending,
      },
    };
  };

  const removeProcessedFacesFromLocal = (processedIds = []) => {
    const cleanProcessedIds = Array.from(
      new Set(
        processedIds
          .map((id) => Number(id))
    ...
    if (cleanProcessedIds.length === 0) return;

    Object.entries(pageCacheRef.current || {}).forEach(([pageKey, cachedData]) => {
      pageCacheRef.current[pageKey] = removeProcessedFacesFromData(
        cachedData,
        cleanProcessedIds
      );
    });
    ...
    setRegistrationData((prev) => {
      const nextData = removeProcessedFacesFromData(prev, cleanProcessedIds);

      if (nextData) {
        const pageKey = String(nextData.pagination?.page || currentPage);
        pageCacheRef.current[pageKey] = nextData;
      }
  ...
  const notifyParentAfterProcessed = (processedIds = []) => {
  ...
  const handleConfirmMember = async () => {
    ...
    try {
      ...
      const processedIds =
        result?.processed_embedding_ids ||
        result?.activated_embedding_ids ||
        recordsToProcess.map((record) => record.id);
      ...
      setModal(null);
      removeProcessedFacesFromLocal(processedIds);
      notifyParentAfterProcessed(processedIds);
  ...
  const handleConfirmReject = async () => {
    ...
    try {
      ...
      const processedIds =
        result?.deleted_embedding_ids ||
        result?.processed_embedding_ids ||
        recordsToProcess.map((record) => record.id);

      const totalDeleted = processedIds.length;
    ...
    <>
      <section className="rounded-3xl border border-amber-200 bg-amber-50 p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <button
            ...
            disabled={isLoading || isPageChanging || isSubmittingAction}
      ...
      {!isLoading && registrationError && (
        <section className="mt-5 flex min-h-[200px] flex-col items-center justify-center rounded-3xl border border-rose-200 bg-rose-50 p-8 text-center shadow-sm">
          ...
          <button
            type="button"
            onClick={() => fetchRegistrationFaces(currentPage, { force: true })}
                    ...
                    {paginationItems.map((item) => {
                      ...
                      return (
                        <button
                          ...
                          title={
                            isCached
                              ? "Page ini sudah tersimpan di cache"
                              : "Request page ini"
                          }
