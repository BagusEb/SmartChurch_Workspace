...
import {
  ...
  Loader2, X, Tag, CalendarDays, ExternalLink,
  Timer, UserCheck, Wifi, Users,
} from 'lucide-react';
...
export default function Attendance() {
  ...
  // stoppedSessionData shape: { sessionName, startTime, apiResult }
  ...
  const [stats,    setStats]    = useState({ known: 0, ambiguous: 0, unknown: 0 });
    ...
    pollingRef.current = setInterval(async () => {
      try {
        ...
        if (data.logs?.length > 0) {
          setLiveLogs(prev => {
            const newLogs = data.logs.filter(nl =>
              !prev.some(ol =>
                ol.time === nl.time &&
                ol.name === nl.name &&
    ...
    (async () => {
      try {
        ...
        if (data.is_running) {
          ...
          setStats(data.stats || { known: 0, ambiguous: 0, unknown: 0 });
          setCurrentSession({ id: data.session_id, name: data.session_name, start_time: null });
          startPolling();
        }
      } catch (e) {
  ...
  const handleConfirmStart = async () => {
    if (!inputName.trim()) { setInputError('Nama sesi wajib diisi'); return; }
    setInputError('');
    setIsStarting(true);
    try {
      ...
      if (res.success) {
        setIsSessionActive(true);
        setCurrentSession({ id: res.session_id, name: res.session_name, start_time: new Date().toISOString() });
        setLiveLogs([]);
        setStats({ known: 0, ambiguous: 0, unknown: 0 });
  ...
  const handleOpenStopConfirm = () => setIsStopConfirmOpen(true);

  const handleConfirmStop = async () => {
    setIsStopConfirmOpen(false);
    setIsStopping(true);

    // Simpan sebelum di-clear — hanya butuh id & name
    ...
    try {
      ...
      if (res.success) {
        ...
        // ── Fetch hasil dari DB setelah sesi ditutup ──
        let apiResult = null;
        if (sessionIdSnap) {
          try {
            ...
            // apiResult tetap null, modal akan tampil dengan fallback
          }
        }
        ...
        setStoppedSessionData({
          ...
          apiResult,   // data murni dari backend, bisa null jika gagal
        });
        setLiveLogs([]);
  ...
  const totalDetections =
    (stats.known || 0) + (stats.ambiguous || 0) + (stats.unknown || 0);
  ...
  return (
    <>
      ...
      <div className="flex flex-col h-full gap-5" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 flex-shrink-0">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0"
              style={{
                background: isSessionActive ? 'linear-gradient(135deg,#10b981,#059669)' : 'linear-gradient(135deg,#6366f1,#8b5cf6)',
                boxShadow:  isSessionActive ? '0 4px 16px rgba(16,185,129,0.35)' : '0 4px 16px rgba(99,102,241,0.35)',
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm mt-0.5">Pantau log deteksi wajah dan statistik absensi secara real-time</p>
            </div>
          </div>
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-semibold flex-shrink-0 transition-all ${
            isSessionActive ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-slate-100 border-slate-200 text-slate-500'
          }`}>
            <span className={`w-2 h-2 rounded-full ${isSessionActive ? 'bg-emerald-500 rec-anim' : 'bg-slate-400'}`} />
            {isSessionActive ? `AKTIF — ${currentSession?.name || 'Sesi Berjalan'}` : 'TIDAK ADA SESI AKTIF'}
          </div>
        </div>
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl text-sm flex items-center justify-between gap-2 flex-shrink-0">
            <div className="flex items-center gap-2"><AlertCircle size={15} className="flex-shrink-0" />{error}</div>
        ...
        <div className="grid grid-cols-3 gap-4 flex-shrink-0">
          <div className="rounded-2xl p-4 text-white" style={{ background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', boxShadow: '0 4px 14px rgba(99,102,241,0.25)' }}>
            ...
            <div className="mt-1.5 flex items-center gap-1">
              ...
              <span className="text-indigo-200 text-xs">Events terekam</span>
            </div>
          </div>
          <div className="rounded-2xl p-4 text-white" style={{ background: 'linear-gradient(135deg,#10b981,#059669)', boxShadow: '0 4px 14px rgba(16,185,129,0.25)' }}>
            <p className="text-emerald-200 text-xs font-semibold uppercase tracking-wide">Dikenali</p>
            <p className="text-3xl font-extrabold mt-1">{stats.known || 0}</p>
            <div className="mt-1.5 w-full bg-emerald-400/30 rounded-full h-1.5">
              <div className="bg-white rounded-full h-1.5 transition-all duration-500"
                style={{ width: totalDetections ? `${Math.round(((stats.known || 0) / totalDetections) * 100)}%` : '0%' }} />
            </div>
          </div>
          <div className="rounded-2xl p-4 text-white" style={{ background: 'linear-gradient(135deg,#f59e0b,#d97706)', boxShadow: '0 4px 14px rgba(245,158,11,0.25)' }}>
            <p className="text-amber-200 text-xs font-semibold uppercase tracking-wide">Perlu Validasi</p>
            <p className="text-3xl font-extrabold mt-1">{(stats.ambiguous || 0) + (stats.unknown || 0)}</p>
            <div className="mt-1.5 flex items-center gap-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 flex-1 min-h-0">
          ...
          <div className="lg:col-span-2 flex flex-col gap-4 min-h-0">
            <div className="flex-1 rounded-2xl overflow-hidden relative border border-slate-800 min-h-[300px] flex flex-col items-center justify-center" style={{ background: '#08080f' }}>
              <div className="absolute top-4 left-4 text-white text-xs px-2.5 py-1 rounded-lg font-mono z-10"
              ...
              {isSessionActive ? (
                  ...
                  <div className="flex flex-col items-center gap-4 text-center px-8 py-6">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <p className="text-emerald-400 font-bold text-sm">Kamera Aktif & Merekam</p>
                      </div>
                      <p className="text-slate-400 text-xs">Sesi: <span className="text-slate-200 font-semibold">{currentSession?.name}</span></p>
                      <p className="text-slate-600 text-xs">AI mendeteksi wajah di latar belakang</p>
                    </div>
                    <div className="flex flex-col items-center gap-2 mt-2">
                      <button onClick={handleOpenCamera} className="btn-camera flex items-center gap-2 text-white px-6 py-3 rounded-xl font-semibold text-sm shadow-lg">
                        <ExternalLink size={16} />Buka Tampilan Kamera Penuh
                      </button>
                      <p className="text-slate-600 text-xs flex items-center gap-1">
                        <Wifi size={11} />Terbuka di tab baru — deteksi tetap berjalan di sini
                      </p>
                    </div>
                    <div className="flex items-center gap-3 mt-1">
                      {[
                        { label: 'Known',     val: stats.known     || 0, color: 'text-emerald-400' },
                        { label: 'Ambiguous', val: stats.ambiguous || 0, color: 'text-amber-400'   },
                        { label: 'Unknown',   val: stats.unknown   || 0, color: 'text-rose-400'    },
                      ].map(s => (
                        <div key={s.label} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl"
                          style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}>
                          <span className={`text-base font-extrabold ${s.color}`}>{s.val}</span>
                ...
                <div className="flex flex-col items-center gap-3 text-center px-8">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 text-xs mt-1">Klik "Mulai Sesi Absensi" untuk mengaktifkan AI dan kamera</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm px-5 py-4 flex items-center justify-between flex-shrink-0">
              <div className="min-w-0 mr-4">
                <p className="text-sm font-bold text-slate-800">Kontrol Sesi Absensi</p>
                <p className="text-xs text-slate-400 mt-0.5 truncate">
                  {isSessionActive ? `Sesi "${currentSession?.name}" sedang berjalan` : 'Pastikan pencahayaan ruangan cukup sebelum memulai'}
                </p>
              </div>
              {!isSessionActive ? (
                <button onClick={handleOpenStartModal} disabled={isStarting}
                ...
                <button onClick={handleOpenStopConfirm} disabled={isStopping}
          ...
          <div className="bg-white border border-slate-100 shadow-sm rounded-2xl flex flex-col overflow-hidden min-h-0">
            <div className="px-4 py-3.5 border-b border-slate-100 flex items-center justify-between flex-shrink-0">
              ...
              <span className="text-xs font-bold px-2.5 py-1 rounded-lg text-indigo-700" style={{ background: 'rgba(99,102,241,0.1)' }}>
                {liveLogs.length}
              </span>
            </div>
              ...
              {liveLogs.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-slate-400 py-12">
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    {isSessionActive ? 'Menunggu wajah terdeteksi kamera...' : 'Mulai sesi untuk memulai perekaman'}
                  </p>
                </div>
              ) : (
                [...liveLogs].reverse().map((log, i) => {
                  const confPct  = Math.round(log.similarity * 100);
                  const isKnown  = log.status === 'KNOWN';
                  const isUpdate = log.is_update;
                  return (
                    <div key={`${log.time}-${log.name}-${log.similarity}-${i}`}
                      ...
                      <div className="mt-0.5 flex-shrink-0">
                        {isKnown
                          ? <div className="w-7 h-7 rounded-lg bg-emerald-50 flex items-center justify-center"><CheckCircle size={14} className="text-emerald-500" /></div>
                          : <div className="w-7 h-7 rounded-lg bg-amber-50 flex items-center justify-center"><AlertCircle size={14} className="text-amber-500" /></div>
                        }
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 min-w-0">
                          ...
                          {isUpdate && (
                            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-indigo-50 text-indigo-500 flex-shrink-0">↑ update</span>
                          )}
                        </div>
                        <div className="flex items-center justify-between mt-1.5">
                          <span className="inline-flex items-center gap-1 text-xs text-slate-400"><Clock size={11} />{log.time}</span>
                          <span className={`text-xs font-mono font-bold px-1.5 py-0.5 rounded-md ${confPct >= 80 ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                            {confPct}%
                          </span>
                        </div>
                      </div>
            ...
            <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50/60 flex-shrink-0 text-center">
              {isSessionActive ? (
                <p className="text-xs text-slate-400 flex items-center justify-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 rec-anim inline-block" />
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="modal-enter bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="relative bg-gradient-to-br from-indigo-500 via-violet-600 to-purple-700 px-6 py-7 text-white">
              <button onClick={() => setIsStartModalOpen(false)}
              ...
              <p className="text-indigo-200 text-sm mt-1">Beri nama sesi ini sebelum kamera diaktifkan.</p>
              <div className="mt-4 flex items-center gap-2 bg-white/10 rounded-xl px-3 py-2 w-fit">
                ...
                <span className="text-indigo-100 text-xs font-semibold">{formatDateLong()}</span>
              </div>
            </div>
            <div className="px-6 py-6 space-y-5">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" placeholder="Contoh: Ibadah Raya Sabat"
                    value={inputName}
                    ...
                    className={`w-full pl-9 pr-4 py-3 border rounded-xl text-sm focus:ring-2 focus:outline-none bg-slate-50 focus:bg-white transition-all placeholder:text-slate-400 ${
                      inputError ? 'border-red-400 bg-red-50 focus:ring-red-300' : 'border-slate-200 focus:ring-indigo-400 focus:border-indigo-400'
                    }`}
                    autoFocus
                  />
                </div>
              ...
              <div>
                <p className="text-[11px] text-slate-400 font-bold mb-2 uppercase tracking-wide">Pilihan Cepat</p>
                <div className="flex flex-wrap gap-2">
                  {['Ibadah Raya Sabat','Ibadah Sabtu Sore','Ibadah Pemuda','Ibadah Doa Malam','Ibadah Anak-anak'].map(s => (
                    <button key={s} onClick={() => { setInputName(s); setInputError(''); }}
            ...
            <div className="flex gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
              ...
              <button onClick={handleConfirmStart} disabled={isStarting}
                ...
                {isStarting ? <Loader2 size={15} className="animate-spin" /> : <Play size={15} fill="currentColor" />}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="modal-enter bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="px-6 pt-8 pb-5 text-center">
              ...
              <h3 className="text-lg font-extrabold text-slate-800">Akhiri Sesi?</h3>
              <p className="text-slate-500 text-sm mt-2 leading-relaxed">
                Kamera akan dimatikan dan sesi{' '}
                <span className="font-bold text-slate-700">"{currentSession?.name}"</span>{' '}
                akan ditutup.
              </p>
            ...
            <div className="flex gap-3 px-6 pb-6">
              <button onClick={() => setIsStopConfirmOpen(false)}
              ...
              <button onClick={handleConfirmStop}
                ...
                <Square size={14} fill="currentColor" />Ya, Akhiri Sesi
              </button>
            </div>
          </div>
        </div>
      ...
      {isStopping && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-2xl px-10 py-9 flex flex-col items-center gap-4 text-center max-w-xs w-full mx-4">
            ...
            <div>
              <p className="font-bold text-slate-800 text-base">Menghentikan Sesi...</p>
              <p className="text-slate-500 text-sm mt-1">Menunggu data tersimpan & mengambil laporan</p>
            </div>
          </div>
        </div>
      )}
...
// ═══════════════════════════════════════════════════════════════
//  SUB-COMPONENT: Stop Result Modal — pure dari API
// ═══════════════════════════════════════════════════════════════
function StopResultModal({ data, onClose }) {
  const { sessionName, startTime, apiResult } = data;

  // Data dari API, fallback jika null
  const sessionNameDisplay  = apiResult?.session_name  || sessionName || 'Sesi Selesai';
  const startTimeDisplay    = apiResult?.start_time    || startTime;
  const endTimeDisplay      = apiResult?.end_time      || null;
  ...
  const attendanceRate = (hasData && typeof presentCount === 'number' && typeof totalActive === 'number' && totalActive > 0)
      ...
      <div className="modal-enter bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="relative bg-gradient-to-br from-emerald-500 to-teal-600 px-6 py-7 text-white flex-shrink-0">
          ...
          <p className="text-emerald-200 text-sm mt-1">
            {hasData ? 'Laporan absensi berhasil diambil dari database.' : 'Data tersimpan. Laporan tidak tersedia saat ini.'}
          </p>
          <div className="mt-4 flex items-center gap-2 bg-white/10 rounded-xl px-3 py-2 w-fit">
            <Tag size={13} className="text-emerald-200" />
            <span className="text-white text-xs font-bold">{sessionNameDisplay}</span>
          </div>
        </div>
        ...
        <div className="px-6 py-5 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-slate-50 rounded-xl p-3.5 border border-slate-100">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-1">Mulai</p>
              <p className="text-sm font-bold text-slate-700 font-mono">{formatTimeShort(startTimeDisplay)}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3.5 border border-slate-100">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-1">Selesai</p>
              <p className="text-sm font-bold text-slate-700 font-mono">{formatTimeShort(endTimeDisplay)}</p>
            </div>
            <div className="bg-slate-50 rounded-xl p-3.5 border border-slate-100">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-1">Durasi</p>
              <div className="flex items-center gap-1">
                <Timer size={12} className="text-indigo-400 flex-shrink-0" />
                <p className="text-sm font-bold text-slate-700">{getDuration(startTimeDisplay, endTimeDisplay)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="rounded-2xl p-4 text-white" style={{ background: 'linear-gradient(135deg,#6366f1,#8b5cf6)' }}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-indigo-200 text-[11px] font-bold uppercase tracking-wide">Member Aktif</p>
              </div>
            ...
            {/* Yang hadir */}
            <div className="rounded-2xl p-4 text-white" style={{ background: 'linear-gradient(135deg,#10b981,#059669)' }}>
              <div className="flex items-center gap-2 mb-2">
                <UserCheck size={14} className="text-emerald-200" />
                <p className="text-emerald-200 text-[11px] font-bold uppercase tracking-wide">Hadir</p>
              </div>
              <p className="text-3xl font-extrabold">{presentCount}</p>
              <p className="text-emerald-200 text-xs mt-1">
                {attendanceRate !== null ? `${attendanceRate}% dari member aktif` : 'Berhasil dikenali AI'}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-[11px] font-bold text-blue-500 uppercase tracking-wide">Terdeteksi</p>
              </div>
              <p className="text-2xl font-extrabold text-blue-700">{totalDetected}</p>
              <p className="text-blue-400 text-xs mt-0.5">Known + Unknown + Ambigu</p>
            </div>
            ...
            <div className="bg-amber-50 border border-amber-100 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-[11px] font-bold text-amber-500 uppercase tracking-wide">Perlu Validasi</p>
              </div>
              <p className="text-2xl font-extrabold text-amber-700">{needValidationCount}</p>
          ...
          {attendanceRate !== null && (
            <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-bold text-slate-600">Tingkat Kehadiran</p>
          ...
          {!hasData && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-bold text-amber-700">Laporan tidak tersedia</p>
